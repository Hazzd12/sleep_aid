#include "Arduino.h"
#include "sleepbreathingradar.h"

#ifdef __AVR__
    #include <SoftwareSerial.h>
    SoftwareSerial SSerial(2, 3); // RX, TX
    #define Serial1 SSerial
#endif

void SleepBreathingRadar::SerialInit(){
  Serial1.begin(9600);
}

// Receive data and process
void SleepBreathingRadar::recvRadarBytes(){
  static boolean recvInProgress = false;
  static byte ndx = 0;
  byte startMarker = MESSAGE_HEAD;            //Header frame
  byte rb; // Each frame received
  while (Serial1.available() > 0 && newData == false) {
      rb = Serial1.read();
      if (recvInProgress == true) {           //Received header frame
          if (dataLen > ndx) {                //Length in range
            Msg[ndx] = rb;
            if(ndx == 0)dataLen = Msg[0];     //Get length frame
            ndx++;
          }
          else {                              //Ending the information acquisition of a set of data
              recvInProgress = false;
              ndx = 0;
              newData = true;
          }
      }
      else if (rb == startMarker) {           //Waiting for the first frame to arrive
          recvInProgress = true;
      }
  }
}

// Unpacking of physical parametersx
String SleepBreathingRadar::Bodysign_judgment(byte inf[], float Move_min, float Move_max){
  typedef union
  {
    unsigned char Byte[4];
    float Float;
  }Float_Byte;
  if(inf[3] == ACTIVE_REPORT){
    if(inf[5] == BODYSIGN){
        Float_Byte fb;
        fb.Byte[0] = inf[6];
        fb.Byte[1] = inf[7];
        fb.Byte[2] = inf[8];
        fb.Byte[3] = inf[9];
        ShowData(inf);
        Serial.println(fb.Float);
        if(fb.Float >= Move_min && fb.Float < Move_max){
          Serial.println("SOMEBODY STOP");
          return "SOMEBOdy_stop";
        }
        else if(fb.Float < Move_min){
          Serial.println("NOBODY");
          return "nobody";
        }
        else if(fb.Float >= Move_max){
          Serial.println("SOMEBODY MOVE");
          return "move";
        }
        Serial.println("----------------------------");
    }
  }
  return "11";
}

//Judgment of occupied and unoccupied, approach and distance
String SleepBreathingRadar::Situation_judgment(byte inf[]){
  String reslut;
  switch(inf[3]){
    case ACTIVE_REPORT:
      switch(inf[4]){
        case REPORT_RADAR:
          switch(inf[5]){
            case ENVIRONMENT:
              ShowData(inf);
              switch(inf[6]){
                case NOBODY:
                  Serial.println("Radar detects no one.");
                  reslut = "no one.";
                  Serial.println("----------------------------");
                  break;
                case SOMEBODY_BE:
                  switch(inf[7]){
                    case SOMEBODY_MOVE:
                      Serial.println("Radar detects somebody is moving.");
                      
                      Serial.println("----------------------------");
                       reslut = "moving";
                      break;
                    case SOMEBODY_STOP:
                      Serial.println("Radar detects somebody is stopping.");
                      Serial.println("----------------------------");
                       reslut = "stopping";
                      break;
                  }
                  break;
              }
              break;
            case HEARTBEAT:
              ShowData(inf);
              switch(inf[6]){
                case NOBODY:
                  Serial.println("Radar detects no one.");
                  Serial.println("----------------------------");
                   reslut = "no one.";
                  break;
                case SOMEBODY_BE:
                  switch(inf[7]){
                    case SOMEBODY_MOVE:
                      Serial.println("Radar detects somebody is moving.");
                      Serial.println("----------------------------");
                       reslut = "moving";
                      break;
                    case SOMEBODY_STOP:
                      Serial.println("Radar detects somebody is stopping.");
                      Serial.println("----------------------------");
                       reslut = "stopping";
                      break;
                  }
                  break;
              }
              break;
            case CLOSE_AWAY:
              ShowData(inf);
              switch(inf[6]){
                case CA_BE:
                  switch(inf[7]){
                    case CA_BE:
                      switch(inf[8]){
                        case CA_BE:
                          Serial.println("Radar detects no move.");
                          Serial.println("----------------------------");
                           reslut = "no one.";
                          break;
                        case CA_CLOSE:
                          Serial.println("Radar detects somebody close.");
                          Serial.println("----------------------------");
                           reslut = "close";
                          break;
                        case CA_AWAY:
                          Serial.println("Radar detects somebody away.");
                          Serial.println("----------------------------");
                           reslut = "away";
                          break;
                      }
                      break;
                  }
                  break;
              }
              break;
            case ABNOEMAL:
              ShowData(inf);
              Serial.println("An exception appears and the reset is complete.");
              Serial.println("----------------------------");
              break;
          }
          break;
        case REPORT_OTHER:
          switch(inf[5]){
            case ENVIRONMENT || HEARTBEAT:
              ShowData(inf);
              switch(inf[6]){
                case NOBODY:
                  Serial.println("Radar detects no one.");
                  Serial.println("----------------------------");
                   reslut = "no_one.";
                  break;
                case SOMEBODY_BE:
                  switch(inf[7]){
                    case SOMEBODY_MOVE:
                      Serial.println("Radar detects somebody is moving.");
                      Serial.println("----------------------------");
                       reslut = "moving";
                      break;
                    case SOMEBODY_STOP:
                      Serial.println("Radar detects somebody is stopping.");
                      Serial.println("----------------------------");
                       reslut = "top";
                      break;
                  }
                  break;
              }
              break;
            case CLOSE_AWAY:
              ShowData(inf);
              switch(inf[6]){
                case CA_BE:
                  switch(inf[7]){
                    case CA_BE:
                      switch(inf[8]){
                        case CA_BE:
                          Serial.println("Radar detects no move.");
                           reslut = "no move";
                          Serial.println("----------------------------");
                          break;
                        case CA_CLOSE:
                          Serial.println("Radar detects somebody close.");
                           reslut = "close";
                          Serial.println("----------------------------");
                          break;
                        case CA_AWAY:
                          Serial.println("Radar detects somebody away.");
                          Serial.println("----------------------------");
                           reslut = "away";
                          break;
                      }
                      break;
                  }
                  break;
              }
              break;
            case ABNOEMAL:
              ShowData(inf);
              Serial.println("An exception appears and the reset is complete.");
              Serial.println("----------------------------");
              break;
          }
          break;
      }
      break;
  }
  return reslut;
}

//Respiratory sleep data frame decoding
String SleepBreathingRadar::Sleep_inf(byte inf[]){
  String result; 
  switch(inf[3]){
    case SLEEP_INF:
      ShowData(inf); // 假设这个函数直接打印了相关信息，而不是返回字符串
      switch(inf[4]){
        case BREATH:
          switch(inf[5]){
            case BREATH_RATE:
              result = "r_rate:" + String(inf[6]);
              Serial.println(result);
              break;
            case CHECK_SIGN:
              switch(inf[6]){
                case BREATH_HOLD:
                 
                  Serial.println( "Abnormal breath-holding detected.");
                  break;
                case BREATH_NULL:
                  
                  Serial.println("No detection signal at the moment.");
                  break;
                case BREATH_NORMAL:
                 
                  Serial.println("Normal breathing was detected.");
                  break;
                case BREATH_MOVE:
                 
                  Serial.println("Abnormal motion is detected.");
                  break;
                case BREATH_RAPID:
                
                  Serial.println( "Abnormal shortness of breath was detected.");
                  break;
              }
              break;
          }
          break;
        case SCENARIO:
          switch(inf[5]){
            case CLOSE_AWAY_BED:
              switch(inf[6]){
                case AWAY_BED:
                  result = "leaving bed.";
                  Serial.println(result);
                  break;
                case CLOSE_BED:
                  result = "in bed.";
                  Serial.println(result);
                  break;
              }
              break;
            case SLEEP_STATE:
              switch(inf[6]){
                case AWAKE:
                  result = "Current: Awake.";
                  Serial.println(result);
                  break;
                case LIGHT_SLEEP:
                  result = "Current: Light sleep.";
                  Serial.println(result);
                  break;
                case DEEP_SLEEP:
                  result = "Current: Deep sleep.";
                  Serial.println(result);
                  break;
                case SLEEP_NULL:
                  result = "Current: NULL.";
                  Serial.println(result);
                  break;
              }
              break;
          }
          break;
        case SLEEP_TIME:
          switch(inf[5]){
            case AWAKE_TIME:
              Serial.print("The user's awake time is detected as: ");
             return "awake time: "+ SleepTimeCalculate(inf[6], inf[7], inf[8], inf[9]);
          break;
              break;
            case LIGHT_SLEEP_TIME:
              Serial.print("The user's light sleep time is detected as: ");
              return " light time: "+ SleepTimeCalculate(inf[6], inf[7], inf[8], inf[9]);
          break;
              break;
            case DEEP_SLEEP_TIME:
              Serial.print("The user's deep sleep time is detected as: ");
              return "deep time: " + SleepTimeCalculate(inf[6], inf[7], inf[8], inf[9]);
          break;
              break;
          }
           
        case SLEEP_QUALITY:
          switch(inf[5]){
            case SLEEP_SCORE:
              result = "Judgment of sleep quality scores: " + String(inf[6]);
              Serial.println(result);
              break;
          }
          break;
      }
      Serial.println("----------------------------");
      break;
  }
  return result; 
}


//Sleep time decoding
String SleepBreathingRadar::SleepTimeCalculate(unsigned char inf1, unsigned char inf2, unsigned char inf3, unsigned char inf4){
  unsigned int rel = 0;
  rel = (inf1 <<  24)+(inf2 << 16 )+(inf3 << 8)+(inf4);
  Serial.println(rel);
  String result = String(rel);
  return result;
}

//Radar transmits data frames for display via serial port
void SleepBreathingRadar::ShowData(byte inf[]){
  for (byte n = 0; n < dataLen+1; n++) {
    Serial.print(inf[n], HEX);
    Serial.print(' ');
  }
    Serial.println();
}

const unsigned char cuc_CRCHi[256]= {
  
};

const unsigned char  cuc_CRCLo[256]= {
  
};

unsigned short int SleepBreathingRadar::us_CalculateCrc16(unsigned char *lpuc_Frame, unsigned short int lus_Len){
  unsigned char luc_CRCHi = 0xFF;
  unsigned char luc_CRCLo = 0xFF;
  int li_Index=0;
  while(lus_Len--){
    li_Index = luc_CRCLo ^ *( lpuc_Frame++);
    luc_CRCLo = (unsigned char)( luc_CRCHi ^ cuc_CRCHi[li_Index]);
    luc_CRCHi = cuc_CRCLo[li_Index];
  }
  return (unsigned short int )(luc_CRCLo << 8 | luc_CRCHi);
}
